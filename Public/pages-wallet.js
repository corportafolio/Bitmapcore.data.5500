...
function WalletDetailPage(props) {
  ...
  var setError = _c[1];
  var _d = React.useState({});
  var parcelPreviewConfs = _d[0];
  var setParcelPreviewConfs = _d[1];
  var _e = React.useState({});
  var bitmapBlockData = _e[0];
  var setBitmapBlockData = _e[1];
  var _eb = React.useState([]);
  var epicBlocks = _eb[0];
  var setEpicBlocks = _eb[1];
  var _f = React.useState({});
  ...
  var _g = React.useState({});
  var bitmapListingPrices = _g[0];
  var setBitmapListingPrices = _g[1];
  ...
  React.useEffect(function() {
    ...
    setIsLoading(true);
    setError(null);
    AssetCache.load(address).then(function(cached) {
      if (cached && cached.collections) {
        setData(cached);
        setIsLoading(false);
      } else {
      ...
      var since = cached && cached.lastHeight ? cached.lastHeight : null;
      return AssetApi.getUserAssets(address, since || undefined).then(function(res) {
        if (res && res.success && res.data) {
          var fresh = {
            collections: res.data.collections,
            total: res.data.total,
          ...
          var merged = cached ? AssetCache.merge(cached, fresh) : fresh;
          AssetCache.save(address, merged);
          setData(merged);
        } else if (!cached) {
          setError(res.error ? res.error.message : 'Error desconocido');
        }
  ...
  React.useEffect(function() {
    if (!data || !data.collections || !address) return;
    var parcelCol = null;
    for (var i = 0; i < data.collections.length; i++) {
      if (data.collections[i].name === 'Parcelas') { parcelCol = data.collections[i]; break; }
    }
    if (!parcelCol || !parcelCol.items || parcelCol.items.length === 0) return;
    var allIds = [];
    parcelCol.items.forEach(function(item) {
      if (item.id) allIds.push(item.id);
    });
    if (allIds.length === 0) return;
    var tryFetch = function(attempt) {
      AssetApi.getBulkParcelConfirmations(allIds, address).then(function(res) {
        if (res && res.success && res.data) {
          setParcelPreviewConfs(function(prev) {
            var updated = {};
            for (var k in prev) updated[k] = prev[k];
    ...
    return React.createElement('div', { className:'p-4 lg:p-6' },
      React.createElement('div', { className:'max-w-4xl mx-auto' },
        React.createElement('div', { className:'flex items-center justify-between mb-4' },
          ...
          React.createElement('span', { className:'font-mono text-sm text-white' }, shortAddr(address))
        ),
        React.createElement('div', { className:'text-center py-12' },
          React.createElement('p', { className:'font-acme text-bitmap-red mb-4' }, error),
          React.createElement('button', {
  ...
  return React.createElement('div', { className:'p-4 lg:p-6' },
    React.createElement('div', { className:'max-w-4xl mx-auto space-y-4' },
      React.createElement('div', { className:'flex items-center justify-between' },
        ...
        React.createElement('h2', { className:'font-alfaslab text-xl text-white' }, shortAddr(address) + ' (' + total + ')')
      ),
      collections.length === 0 ? React.createElement('div', { className:'text-center py-12' },
        React.createElement('p', { className:'font-acme text-bitmap-muted mb-4' }, 'No se encontraron activos')
      ) :
      React.createElement('div', { className:'space-y-3' },
        collections.map(function(col) {
          ...
          return React.createElement('div', {
            key:col.name,
            className:'bg-bitmap-surface border border-bitmap-border rounded-xl p-4'
          },
            React.createElement('div', { className:'flex items-center justify-between mb-3' },
              React.createElement('span', {
                className:'font-alfaslab text-base',
            ...
            React.createElement('div', { className:'flex items-start gap-3 overflow-x-auto pb-2 parcel-scrollbar' },
              col.items.slice().sort(function(a, b) {
                if (col.name === 'Parcelas') {
                  var ca = parcelPreviewConfs[a.id];
                  var cb = parcelPreviewConfs[b.id];
